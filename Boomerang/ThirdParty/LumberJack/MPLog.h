...
**/
    ...
    #define LOG_LEVEL_DEF mpLogLevel
#endif

@class MPLogMessage;

@protocol MPLogger;
@protocol MPLogFormatter;
...
#define MPLogError(frmt, ...)   LOG_OBJC_MAYBE(LOG_ASYNC_ERROR,   LOG_LEVEL_DEF, LOG_FLAG_ERROR,   0, frmt, ##__VA_ARGS__)
#define MPLogWarn(frmt, ...)    LOG_OBJC_MAYBE(LOG_ASYNC_WARN,    LOG_LEVEL_DEF, LOG_FLAG_WARN,    0, frmt, ##__VA_ARGS__)
#define MPLogInfo(frmt, ...)    LOG_OBJC_MAYBE(LOG_ASYNC_INFO,    LOG_LEVEL_DEF, LOG_FLAG_INFO,    0, frmt, ##__VA_ARGS__)
#define MPLogDebug(frmt, ...)   LOG_OBJC_MAYBE(LOG_ASYNC_DEBUG,   LOG_LEVEL_DEF, LOG_FLAG_DEBUG,   0, frmt, ##__VA_ARGS__)
#define MPLogVerbose(frmt, ...) LOG_OBJC_MAYBE(LOG_ASYNC_VERBOSE, LOG_LEVEL_DEF, LOG_FLAG_VERBOSE, 0, frmt, ##__VA_ARGS__)

#define MPLogCError(frmt, ...)   LOG_C_MAYBE(LOG_ASYNC_ERROR,   LOG_LEVEL_DEF, LOG_FLAG_ERROR,   0, frmt, ##__VA_ARGS__)
#define MPLogCWarn(frmt, ...)    LOG_C_MAYBE(LOG_ASYNC_WARN,    LOG_LEVEL_DEF, LOG_FLAG_WARN,    0, frmt, ##__VA_ARGS__)
#define MPLogCInfo(frmt, ...)    LOG_C_MAYBE(LOG_ASYNC_INFO,    LOG_LEVEL_DEF, LOG_FLAG_INFO,    0, frmt, ##__VA_ARGS__)
#define MPLogCDebug(frmt, ...)   LOG_C_MAYBE(LOG_ASYNC_DEBUG,   LOG_LEVEL_DEF, LOG_FLAG_DEBUG,   0, frmt, ##__VA_ARGS__)
#define MPLogCVerbose(frmt, ...) LOG_C_MAYBE(LOG_ASYNC_VERBOSE, LOG_LEVEL_DEF, LOG_FLAG_VERBOSE, 0, frmt, ##__VA_ARGS__)
...
#define THIS_FILE (MPExtractFileNameWithoutExtension(__FILE__, NO))
...
+ (void)addLogger:(id <MPLogger>)logger;    // adds the logger using maximum log level (LOG_LEVEL_VERBOSE)
...
- (void)setLogFormatter:(id <MPLogFormatter>)formatter;
...
@protocol MPLogFormatter <NSObject>
@required
...
**/
- (NSString *)formatLogMessage:(MPLogMessage *)logMessage;
...
**/
- (void)didAddToLogger:(id <MPLogger>)logger;
- (void)willRemoveFromLogger:(id <MPLogger>)logger;
...
+ (int)mpLogLevel;
+ (void)mpSetLogLevel:(int)logLevel;
...
enum {
    MPLogMessageCopyFile     = 1 << 0,
    MPLogMessageCopyFunction = 1 << 1
};
typedef int MPLogMessageOptions;

@interface MPLogMessage : NSObject <NSCopying>
{
...
@interface MPAbstractLogger : NSObject <MPLogger>
{
    id <MPLogFormatter> formatter;
