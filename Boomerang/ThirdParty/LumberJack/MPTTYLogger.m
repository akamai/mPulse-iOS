...
@interface MPTTYLoggerColorProfile : NSObject {
@public
    int mask;
    int context;
...
static MPTTYLogger *sharedInstance;
...
+ (NSUInteger)codeIndexForColor:(OSColor *)inColor
{
    ...
    for (OSColor *color in colors)
    {
        ...
        NSLogVerbose(@"MPTTYLogger: %3lu : %.3f,%.3f,%.3f & %.3f,%.3f,%.3f = %.6f",
        ...
        if (distance < lowestDistance)
        {
            ...
            NSLogVerbose(@"MPTTYLogger: New best index = %lu", (unsigned long)bestIndex);
        }
...
{
    static dispatch_once_t MPTTYLoggerOnceToken;
    dispatch_once(&MPTTYLoggerOnceToken, ^{
        ...
        NSLogInfo(@"MPTTYLogger: isaColorTTY = %@", (isaColorTTY ? @"YES" : @"NO"));
        NSLogInfo(@"MPTTYLogger: isaColor256TTY: %@", (isaColor256TTY ? @"YES" : @"NO"));
        NSLogInfo(@"MPTTYLogger: isaXcodeColorTTY: %@", (isaXcodeColorTTY ? @"YES" : @"NO"));
...
- (id)init
{
    ...
    if ((self = [super init]))
    {
        ...
        calendarUnitFlags = (kCFCalendarUnitYear     |
...
- (void)setForegroundColor:(OSColor *)txtColor backgroundColor:(OSColor *)bgColor forFlag:(int)mask context:(int)ctxt
{
    dispatch_block_t block = ^{ @autoreleasepool {
        ...
        MPTTYLoggerColorProfile *newColorProfile =
            [[MPTTYLoggerColorProfile alloc] initWithForegroundColor:txtColor
                                                     backgroundColor:bgColor
                                                                flag:mask
                                                             context:ctxt];
        
        NSLogInfo(@"MPTTYLogger: newColorProfile: %@", newColorProfile);
        
        NSUInteger i = 0;
        for (MPTTYLoggerColorProfile *colorProfile in colorProfilesArray)
        {
...
- (void)logMessage:(MPLogMessage *)logMessage
{
    ...
    if (logMsg)
    {
        ...
        MPTTYLoggerColorProfile *colorProfile = nil;
        ...
        if (colorsEnabled)
        {
            ...
            if (colorProfile == nil)
            {
                for (MPTTYLoggerColorProfile *cp in colorProfilesArray)
                {
...
@implementation MPTTYLoggerColorProfile
...
- (instancetype)initWithForegroundColor:(OSColor *)fgColor backgroundColor:(OSColor *)bgColor flag:(int)aMask context:(int)ctxt
{
    if ((self = [super init]))
    {
        ...
        if (fgColor)
        {
            [MPTTYLogger getRed:&r green:&g blue:&b fromColor:fgColor];
        ...
        if (bgColor)
        {
            [MPTTYLogger getRed:&r green:&g blue:&b fromColor:bgColor];
        ...
        if (fgColor && isaColorTTY)
        {
            ...
            fgCodeIndex = [MPTTYLogger codeIndexForColor:fgColor];
        ...
        if (bgColor && isaColorTTY)
        {
            ...
            bgCodeIndex = [MPTTYLogger codeIndexForColor:bgColor];
...
- (NSString *)description
{
    return [NSString stringWithFormat:
            @"<MPTTYLoggerColorProfile: %p mask:%i ctxt:%i fg:%u,%u,%u bg:%u,%u,%u fgCode:%@ bgCode:%@>",
