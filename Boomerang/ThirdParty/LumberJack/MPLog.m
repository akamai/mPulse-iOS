...
#define NSLogDebug(frmt, ...) do{ if(MP_DEBUG) NSLog((frmt), ##__VA_ARGS__); } while(0)
...
@interface MPLoggerNode : NSObject {
@public 
    id <MPLogger> logger;   
...
+ (MPLoggerNode *)nodeWithLogger:(id <MPLogger>)logger loggerQueue:(dispatch_queue_t)loggerQueue logLevel:(int)logLevel;

@end


@interface MPLog (PrivateAPI)

+ (void)lt_addLogger:(id <MPLogger>)logger logLevel:(int)logLevel;
+ (void)lt_removeLogger:(id <MPLogger>)logger;
+ (void)lt_removeAllLoggers;
+ (NSArray *)lt_allLoggers;
+ (void)lt_log:(MPLogMessage *)logMessage;
+ (void)lt_flush;
...
+ (void)initialize
{
    static dispatch_once_t MPLogOnceToken;
    dispatch_once(&MPLogOnceToken, ^{
        
        loggers = [[NSMutableArray alloc] initWithCapacity:4];
        
        NSLogDebug(@"MPLog: Using grand central dispatch");
        ...
        NSLogDebug(@"MPLog: numProcessors = %u", numProcessors);
...
+ (void)addLogger:(id <MPLogger>)logger withLogLevel:(int)logLevel
{
...
+ (void)queueLogMessage:(MPLogMessage *)logMessage asynchronously:(BOOL)asyncFlag
{
     ...
     format:(NSString *)format, ...
{
    ...
    if (format)
    {
        ...
        MPLogMessage *logMessage = [[MPLogMessage alloc] initWithLogMsg:logMsg
                                                                  level:level
                                                                   flag:flag
                                                                context:context
                                                                   file:file
...
+ (void)log:(BOOL)asynchronous
    message:(MPLogMessage *)logMessage
{
    [self queueLogMessage:logMessage asynchronously:asynchronous];
}
...
{
    SEL getterSel = @selector(mpLogLevel);
    SEL setterSel = @selector(mpSetLogLevel:);
...
+ (void)lt_addLogger:(id <MPLogger>)logger logLevel:(int)logLevel
{
    ...
    MPLoggerNode *loggerNode = [MPLoggerNode nodeWithLogger:logger loggerQueue:loggerQueue logLevel:logLevel];
...
+ (void)lt_removeLogger:(id <MPLogger>)logger
{
    ...
    MPLoggerNode *loggerNode = nil;
    
    for (MPLoggerNode *node in loggers)
    {
    ...
    if (loggerNode == nil)
    {
        NSLogDebug(@"MPLog: Request to remove logger which wasn't added");
        return;
    }
...
+ (void)lt_removeAllLoggers
{
    ...
    for (MPLoggerNode *loggerNode in loggers)
    {
...
NSString *MPExtractFileNameWithoutExtension(const char *filePath, BOOL copy)
{
...
@implementation MPLoggerNode

@synthesize logLevel;

- (instancetype)initWithLogger:(id <MPLogger>)aLogger loggerQueue:(dispatch_queue_t)aLoggerQueue logLevel:(int)aLogLevel
{
    if ((self = [super init]))
    {
        logger = aLogger;
...
+ (MPLoggerNode *)nodeWithLogger:(id <MPLogger>)logger loggerQueue:(dispatch_queue_t)loggerQueue logLevel:(int)logLevel
{
    return [[MPLoggerNode alloc] initWithLogger:logger loggerQueue:loggerQueue logLevel:logLevel];
}
...
@implementation MPLogMessage

static char *mp_str_copy(const char *str)
{
...
- (instancetype)initWithLogMsg:(NSString *)msg
                      ...
                      function:(const char *)aFunction
                          line:(int)line
                           tag:(id)aTag
                       options:(MPLogMessageOptions)optionsMask
{
                     ...
                     timestamp:(NSDate *)aTimestamp
{
  if ((self = [super init]))
  {
    ...
    if (options & MPLogMessageCopyFile)
      file = mp_str_copy(aFile);
    else
      file = (char *)aFile;

    if (options & MPLogMessageCopyFunction)
      function = mp_str_copy(aFunction);
    else
...
#define NSFoundationVersionNumber_iOS_6_1 993.00
      ...
      queueLabel = mp_str_copy(dispatch_queue_get_label(DISPATCH_CURRENT_QUEUE_LABEL));
      gotLabel = YES;
    }
#endif
...
#pragma clang diagnostic pop
      ...
      queueLabel = mp_str_copy(dispatch_queue_get_label(currentQueue));
      gotLabel = YES;
    }

    // c) Give up
    if (!gotLabel) {
      queueLabel = mp_str_copy(""); // iOS 6.x only
    }
...
- (NSString *)fileName
{
    return MPExtractFileNameWithoutExtension(file, NO);
}
...
- (void)dealloc
{
    if (file && (options & MPLogMessageCopyFile))
        free(file);

    if (function && (options & MPLogMessageCopyFunction))
        free(function);
...
- (id)copyWithZone:(NSZone *)zone {
    MPLogMessage *newMessage = [[MPLogMessage alloc] init];
    ...
    if (self->options & MPLogMessageCopyFile) {
        newMessage->file = mp_str_copy(self->file);
        newMessage->function = mp_str_copy(self->function);
    } else {
    ...
    newMessage->queueLabel = mp_str_copy(self->queueLabel);
...
@implementation MPAbstractLogger
...
- (void)logMessage:(MPLogMessage *)logMessage
{
    // Override me
}

- (id <MPLogFormatter>)logFormatter
{
    ...
    dispatch_queue_t globalLoggingQueue = [MPLog loggingQueue];
    
    __block id <MPLogFormatter> result;
...
- (void)setLogFormatter:(id <MPLogFormatter>)logFormatter
{
